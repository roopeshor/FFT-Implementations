...
module butterfly_structural #(
    ...
    // Internal wires to connect the instantiated blocks
    logic signed [WIDTH-1:0] p_rr, p_ii, p_ri, p_ir;
    logic signed [WIDTH-1:0] bw_re, bw_im;

    // ---------------------------------------------------------------------
    // Step 1: Complex Multiplication (4 Multipliers)
    // ---------------------------------------------------------------------
    ...
    // ---------------------------------------------------------------------
    // Step 2: Resolve the Complex Multiplication (1 Adder, 1 Subtractor)
    ...
    // ---------------------------------------------------------------------
    // Step 3: Butterfly Add/Sub (2 Adders, 2 Subtractors)
    // ---------------------------------------------------------------------
    // c.re = a.re + bw_re
